...
describe('PackagesController', () => {
  ...
  describe('detail', () => {
    ...
    it('passes hasManage=false for member role', async () => {
      ...
      const memberUser: AuthenticatedUser = { userId: BigInt(2), email: 'm@t.com', roles: ['member'] }
  ...
  describe('updateStatus', () => {
    it('delegates to updatePackageStatus and wraps success', async () => {
      ...
      expect(mockService.updatePackageStatus).toHaveBeenCalledWith(BigInt(5), 'inactive', user.userId)
      expect(res).toEqual({ success: true, ...serviceResult })
    })
  })
