...
export default function RenewPaymentPage() {
  ...
  useEffect(() => {
    if (!state?.packageId) navigate('/member/subscription/renew', { replace: true })
  }, [state, navigate])
  ...
  const renewStart = new Date(state.renewStart)
  const renewEnd   = new Date(renewStart.getTime() + Number(state.durationDays) * 86400000)
  ...
  return (
    <div style={{ fontFamily: "'Be Vietnam Pro',sans-serif", maxWidth: 960, margin: '0 auto' }}>
      ...
      >
        <div>
          <p style={{ fontSize: 12, color: '#bbcabf', marginBottom: 2 }}>Gia hạn gói</p>
          ...
          <p style={{ fontSize: 12, color: '#bbcabf', marginTop: 2 }}>
            {state.durationDays} ngày &nbsp;·&nbsp; {fmtDate(renewStart)} → {fmtDate(renewEnd)}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: BG, border: '1px solid rgba(66,224,158,0.08)' }}>
          ...
          >
            {submitting ? 'Đang xử lý...' : `Xác nhận gia hạn ${fmtVND(state.price)}`}
          </button>
        </div>
