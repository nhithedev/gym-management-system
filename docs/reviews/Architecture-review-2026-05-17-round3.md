...
|---|---|---|---|
| READ-M01 | §3.3 line 253 | `photo_url` = "signed URL Supabase Storage TTL 5 phút" — không có SDK call pattern. Dev phải tự tìm Supabase docs. | OPEN |
| READ-M02 | §4.1.4 line 323 | Rate limit forgot-password 3/h/email — không nói implementation: Redis? In-memory Map? NestJS throttler? | OPEN |
| READ-M03 | §5.2 line 530 | `subscription:cancel-unpaid-pending` "không có payment success" — undefined: `payments.status='paid'`? `='success'`? Phải sang Database.md. | OPEN |
| READ-M04 | §4.4.1 + §4.1.4 | Không rõ `auth.login` audit ghi cả failed login hay chỉ success. 2 section nói khác nhau. | OPEN |
| READ-M05 | §6.1 NFR | "Đo bằng: Health check + sample endpoint" — không tool (k6/wrk/curl), không endpoint critical path. Không actionable v1.0. | OPEN |
