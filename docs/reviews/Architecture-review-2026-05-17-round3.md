...
| READ-M04 | §4.4.1 + §4.1.4 | Không rõ `auth.login` audit ghi cả failed login hay chỉ success. 2 section nói khác nhau. | FIXED (phase 8) — §4.4.1 row `auth.login` clarify ghi CẢ success lẫn failed với payload `{success: boolean, reason?: 'invalid_credentials'\|'email_not_verified'\|'user_disabled'}`; §4.4.2 thêm "Failed login exception" — interceptor catch 401 trước propagate, `actor_user_id=NULL` khi credential không khớp, lưu `payload.email_attempted` cho brute-force forensics. Unblock Module 1 Auth API spec. |
