...
| LOG-M03 | Arch §5.2 line 527; SRS UC03B step 8a | "Sau 24h cron auto-cancel" — cron daily 00:15 giao 24h-48h window thực tế (subscription tạo 00:16 day D bị cancel 00:15 day D+2 = ~48h). Spec SRS nói "24h" — mismatch. | FIXED (phase 8) — Arch §5.2 cron row clarify "Cửa sổ thực tế 24-48 giờ" + ví dụ 2 boundary; SRS UC03B step 8a sync "24-48h". Defer cron hourly v1.1+ nếu business cần window chặt hơn. |
| LOG-M04 | Arch §4.3.2 line 416; SRS UC04B step 4 | UC04B cancel subscription `active` + có `pending` prepaid → activate prepaid `start_date=CURRENT_DATE` recompute `end_date`. Cascading logic không document trong Architecture. Dev đọc Architecture only sẽ miss. Cần add §4.3.3 với `$transaction` requirement + dùng `today_vn`. | FIXED (phase 8) — Arch §4.3.3 mới với `$transaction` code sample (cancel active + activate pending + 2 audit logs cùng tx), dùng `today_vn`, race handling qua P2025. SRS UC04B step 4 đã có từ phase 7. |
...
| LOG-M07 | Arch §5.2 line 526 | Cron `subscription:activate-pending` "đã payment success" không spec query join. Cần `EXISTS (SELECT 1 FROM payments WHERE subscription_id=? AND status='success')` + composite index `(subscription_id, status)` (chưa có trong schema.prisma). | FIXED (phase 8) — Arch §5.2 cron row thêm index requirement note: composite index `@@index([subscriptionId, status])` defer khi implement Module 4 Subscription (pattern design-stability-first, không touch Prisma schema phase 8 doc-only). `EXISTS` query đã có từ phase 6. |
