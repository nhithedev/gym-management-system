...
Hệ thống gồm 20 bảng, được nhóm theo chủ đề nghiệp vụ như sau:
...
**Ràng buộc:** UNIQUE `(staff_id, shift, work_date)` ngăn cùng 1 nhân sự đăng ký trùng ca trùng ngày.
...
- Schema dùng `DECIMAL(12,2)` để có buffer mở rộng đa tiền tệ ở v1.x, nhưng v1.0 chỉ lưu giá nguyên (phần thập phân luôn `.00`)
...
CREATE TABLE members (
    ...
    CONSTRAINT fk_members_primary_trainer
...
CREATE TABLE staff_schedules (
    ...
    CONSTRAINT fk_schedules_staff
        FOREIGN KEY (staff_id) REFERENCES staff(staff_id),
    CONSTRAINT uq_schedules_staff_shift_date UNIQUE (staff_id, shift, work_date)
);
