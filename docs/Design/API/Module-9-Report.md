...
Tất cả 7 endpoint dùng chung 2 query param sau:
...
| `/reports/employee-performance` | `staff`, `training_sessions` | `staff.deleted_at IS NULL`, `sessions.status='completed'`, `sessions.start_time BETWEEN from AND to` |
| `/reports/employee-performance/:staffId/detail` | `staff`, `training_sessions` | `staff.staff_id={staffId}`, `staff.deleted_at IS NULL`, `sessions.status='completed'`, `sessions.start_time BETWEEN from AND to`; trả breakdown theo ngày |
| `/reports/top-packages` | `subscriptions`, `packages` | `subscriptions.created_at BETWEEN from AND to`, join `packages` cho thông tin gói, SUM `subscription.price` cho revenue |
...
Tất cả 7 endpoint đều read-only. Theo conventions.md §18: "KHÔNG log GET". Không có audit action code nào được dùng trong Module 9.
